...
template <typename T>
void Swap(T* first, T* second) // обменивает местами значения, на которые указывают указатели first и second
{
    T temp = *first;
    *first = *second;
    *second = temp;
...
template <typename T>
void SortPointers(std::vector<T*>& pointers) // сортирует указатели по значениям, на которые они указывают
{
